...
struct AllenOverlapFormula : public Formula {
  std::unique_ptr<Formula> left;
  std::unique_ptr<Formula> right;

  AllenOverlapFormula(std::unique_ptr<Formula>&& left,
                std::unique_ptr<Formula>&& right)
    : left(std::move(left)), right(std::move(right)) {}

  ~AllenOverlapFormula() override = default;

  std::unique_ptr<Formula> clone() const override {
    return std::make_unique<AllenOverlapFormula>(left->clone(), right->clone());
  }

  bool operator==(const AllenOverlapFormula& other) const {
    return left->equals(other.left.get()) && right->equals(other.right.get());
  }

  bool equals(Formula* other) const override {
    if (auto other_formula = dynamic_cast<AllenOverlapFormula*>(other)) {
