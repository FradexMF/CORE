...
struct AllenStartsFormula : public Formula {
  std::unique_ptr<Formula> left;
  std::unique_ptr<Formula> right;

  AllenStartsFormula(std::unique_ptr<Formula>&& left,
                std::unique_ptr<Formula>&& right)
    : left(std::move(left)), right(std::move(right)) {}

  ~AllenStartsFormula() override = default;

  std::unique_ptr<Formula> clone() const override {
    return std::make_unique<AllenStartsFormula>(left->clone(), right->clone());
  }

  bool operator==(const AllenStartsFormula& other) const {
    return left->equals(other.left.get()) && right->equals(other.right.get());
  }

  bool equals(Formula* other) const override {
    if (auto other_formula = dynamic_cast<AllenStartsFormula*>(other)) {
  ...
  std::string to_string() const override {
    return "(" + left->to_string() + " :s " + right->to_string() + ")";
  }
