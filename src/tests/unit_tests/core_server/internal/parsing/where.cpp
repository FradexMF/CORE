...
TEST_CASE("event :o event", "[Where]") {
  ...
  auto expected_formula = make_unique<AllenOverlapFormula>(
                          make_unique<EventTypeFormula>("H"), 
                          make_unique<EventTypeFormula>("T"));
  auto formula = parse_formula(query);
  INFO("Expected: " + expected_formula->to_string());
...
TEST_CASE("event :s event", "[Where]") {
  auto query = create_where_query("H:sT");
...
TEST_CASE("event :d event 2", "[Where]") {
  auto query = create_where_query("H :d T");
  auto expected_formula = make_unique<AllenDuringFormula>(
                          make_unique<EventTypeFormula>("H"), 
                          make_unique<EventTypeFormula>("T"));
  auto formula = parse_formula(query);
  INFO("Expected: " + expected_formula->to_string());
...
TEST_CASE("event :f event 2", "[Where]") {
  auto query = create_where_query("H :f T");
  auto expected_formula = make_unique<AllenFinishesFormula>(
                          make_unique<EventTypeFormula>("H"), 
                          make_unique<EventTypeFormula>("T"));
  auto formula = parse_formula(query);
  INFO("Expected: " + expected_formula->to_string());
